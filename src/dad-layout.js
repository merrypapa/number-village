...
const SLAB = [
  ...
  //  ★ 계단참은 **넓게**. 좁으면 2층에서 내려가는 길을 못 찾는다
  //    (아이가 "내려가는 계단이 없어"라고 알려줬다 — 서남쪽 구석에만 입구가 있었다)
  //  ★ 계단참의 폭을 **계단 폭과 똑같이** 맞춘다.
  //    더 넓게 두면 계단 옆에 막다른 자리가 생겨서 "내려가는 길이 없다"고 느낀다
  { x0: -DAD_W / 2, x1: -15, z0: -6, z1: 1 },           // 계단이 올라오는 자리(계단참)
];
...
//  ★ 계단은 **넓게**. 2층에서 남쪽으로 걸으면 그냥 계단을 만나야 한다.
//    단, x1이 -14.4보다 오른쪽으로 가면 1층 기차 레일(반지름 12+2.4)과 겹친다
const STAIR = { x0: -26, x1: -15, zBot: 15, zTop: 1, steps: 22 };
...
export function buildDadStructure(scene) {
  ...
  //  ★ 계단(x -26~-17, z 1~15) 위에 기둥을 세우면 계단을 오르다 막힌다
  //  ★ 기차 레일(가운데에서 반지름 12) 위에도 세우면 안 된다.
  //    z -9에 세웠더니 (±8, -9)이 딱 레일 위여서 기차가 기둥을 뚫고 지나갔다
  for (const x of [-22, -8, 8, 22]) {
    for (const z of [-21, -13]) {
      pillar(g, x, z, DAD_F2);
      obstacles.push({ x, z, r: 1.1, ...DF1 });
    }
  }
  ...
  // --- 2층 난간 (떨어지지 않게) ---
  railing(g, obstacles, -15, -6, DAD_W / 2, -6);     // 캠핑 데크 남쪽 (계단 입구만 뚫려 있다)
  railing(g, obstacles, -DAD_W / 2, 1, -26, 1);      // 계단참 남쪽 (계단 서쪽 자투리)
  railing(g, obstacles, -15, -6, -15, 1);            // 계단참 동쪽
