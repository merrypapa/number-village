...
//  ★ 아이랑 같이 바꿔볼 값
// -----------------------------------------------------------
const WALL_X    = -18.8;   // 친구들이 서 있는 줄의 x 자리 (서쪽 벽 앞)
const ROW_HALF  = 17;      // 줄의 길이 절반 (z = -17 ~ 17)
 ...
 */
export function buildGallery(playerCharId) {
  ...
  // 진열대 앞에 깔린 붉은 융단
  const carpet = part('box', C.red, WALL_X + 0.8, 0.05, 0, 5.6, 0.1, ROW_HALF * 2 + 4);
  ...
  for (let i = 0; i < n; i++) {
    const def = defs[i];
    const z = n === 1 ? 0 : -ROW_HALF + (i / (n - 1)) * ROW_HALF * 2;
    ...
    obstacles.push({ x: WALL_X, z, r: 1.25 });
